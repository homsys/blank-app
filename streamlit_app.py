...
def main():
    ...
    # Пример извлечения параметра tgWebAppData
    tg_web_app_data = query_params.get("tgWebAppData", [None])[0]

    if tg_web_app_data:
        st.write(f"Telegram Web App Data: {tg_web_app_data}")
        # Здесь можно добавить логику для обработки данных пользователя
    else:
        st.write("No Telegram Web App Data found in URL.")
